...
if ($chat != "" || $attachedFile['name'] != "") {
    if ($attachedFile['name'] != "") {
        ...
        if (move_uploaded_file($attachedFile['tmp_name'], "media/$uploadfile")) {
            $sql = mysqli_query($connect, "INSERT chat(sender_id, send_to, content, sender_type) VALUES('$sender', '$sendTo', '$encryptFile', 'atty')");
        }
    }
    if ($chat != "") {
        $encryptChat = encrypt_decrypt($_POST['chat'], 'encrypt');
        $sql = mysqli_query($connect, "INSERT chat(sender_id, send_to, content, sender_type) VALUES('$sender', '$sendTo', '$encryptChat', 'atty')");
    }
}
